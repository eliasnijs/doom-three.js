...
import { Grid, Pos, drawLine, drawSquare } from '../utils/generate-maze.ts'
...
export class MazePanel extends GameObject {
	...
	constructor(state: State, renderer: WebGLRenderer, style?: Record<string, string>) {
		super(state)
		...
		for (let row = 0; row < grid.nRows; row++) {
			for (let col = 0; col < grid.nCols; col++) {
				...
				if (cell.walls[1]) { // East
					drawLine(this.container, x + cellWidth, y, x + cellWidth, y + cellHeight, this.lineWidth, this.colors.wallColor)
				}
				if (cell.walls[2]) { // South
					drawLine(this.container, x, y + cellHeight, x + cellWidth, y + cellHeight, this.lineWidth, this.colors.wallColor)
				}
