...
\subsection{Lighting}
The game uses a global ambient light for base illumination. Hallway light fixtures use diffuse materials to simulate lights, providing something that looks like a light, even in the reflections, without requiring expensive dynamic lighting calculations. Figure \ref{fig:lights} shows an example of these light fixtures in the game.
...
We use Three.js’s default frustum culling to improve performance by automatically skipping objects outside the camera’s view. This built-in feature checks each object’s bounding volume against the camera’s frustum and omits rendering if it’s not visible, reducing unnecessary GPU processing.
